...
export async function handleApproverAPI(request: Request, path: string, ipAddress: string): Promise<Response> {
  const authResult = await RoleMiddleware.checkAuthAndRole(request, ipAddress, UserRole.APPROVER);
  ...
  try {
    ...
    if (method === 'GET') {
      if (apiPath === 'pending-count') {
        const result = db.query("SELECT COUNT(*) as count FROM aft_requests WHERE status = 'pending_approval'").get() as any;
    ...
    if (method === 'POST') {
      ...
      if (apiPath.startsWith('approve/')) {
        ...
        `).run(requestId, session.email, notes || 'Request approved');
